...
from common.permissions import AdminUserRequiredMixin, NavbarManagementMixin
...
class UserListView(NavbarManagementMixin, TemplateView):
...
class UserUpdateView(AdminUserRequiredMixin, SuccessMessageMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        check_rules = get_password_check_rules()
        arr = []
        for n in Navbar.objects.filter(parent_id=None):
            children = n.child_navbars.all().order_by('sort')
            if children:
                arr.append([n.id,list(children.values_list('id', flat=True))])
    ...
    def form_valid(self, form):
        if form.instance.is_superuser and not self.request.user.is_superuser:
        ...
        user.navbars.add(*list(form.cleaned_data['navbars']))
        user.dashboards.add(*list(form.cleaned_data['dashboards']))
        user.save()
...
class UserDetailView(AdminUserRequiredMixin, DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not request.user.is_superadmin and self.object.is_superadmin:
        ...
        for row in csv_data[1:]:
            ...
            for k, v in user_dict.items():
                if k in ['is_active']:
                    ...
                    v = UserRole.objects.get(id=int(v))
                else:
                    continue
                user_dict[k] = v
